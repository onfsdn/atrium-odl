...
public class OpenFlowDeviceDriver implements OpenflowFeatureService,
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(OpenFlowDeviceDriver.class);
    private static final InstanceIdentifier<DeviceType> PATH = InstanceIdentifier
    ...
    public OpenFlowDeviceDriver(DataBroker dataBroker,
    ...
    public Future<RpcResult<AdjustFlowOutput>> adjustFlow(AdjustFlowInput input) {
        ...
        List<Flow> adjustedFlows = ImmutableList.of(new FlowBuilder(input
                .getFlow()).build());

        AdjustFlowOutput output = new AdjustFlowOutputBuilder().setFlow(
                adjustedFlows).build();
        return Futures
                .immediateFuture(RpcResultBuilder.success(output).build());
    }
    ...
    public Future<RpcResult<Void>> forward(ForwardInput input) {
        ...
        case Add:
            ...
            DriverUtil.install_flows(this.salFlowService, flows,
                    (InstanceIdentifier<Node>) input.getNode().getValue());
            break;
        case Remove:
            // Delete operation delete the flows.
            DriverUtil.deleteFlows(this.salFlowService, flows,
                    (InstanceIdentifier<Node>) input.getNode().getValue());
            break;

        default:
            // don't do anything.
            LOG.warn("Operation {} not supported", input
                    .getForwardingObjective().getOperation());
            break;
        }
    ...
    public void onDataChanged(
        ...
        Map<InstanceIdentifier<?>, DataObject> createdData = change
                .getCreatedData();
        if (createdData != null) {
            for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : createdData
                    .entrySet()) {
                DeviceType deviceType = (DeviceType) entry.getValue();
                if (isMininetDeviceType(deviceType.getDeviceType())) {
                    registerRpcService(entry.getKey().firstIdentifierOf(
                            Node.class));
                }
            }
        }
        ...
        if ((removedPaths != null) && !removedPaths.isEmpty()) {
            for (InstanceIdentifier<?> removedPath : removedPaths) {
                DeviceType deviceType = (DeviceType) change.getOriginalData()
                        .get(removedPath);
                if (isMininetDeviceType(deviceType.getDeviceType())) {
                    closeRpcRegistration(removedPath
    ...
    private static boolean isMininetDeviceType(
            Class<? extends DeviceTypeBase> deviceType) {
            ...
            try {
                BindingAwareBroker.RoutedRpcRegistration<OpenflowFeatureService> registration = rpcRegistry
                        .addRoutedRpcImplementation(
                                OpenflowFeatureService.class, this);
    ...
    private Collection<org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.tables.table.Flow> processSpecific(
        ...
        try {
            ...
            Action action = fwd.getAction();
            Long etherType = match.getEthernetMatch().getEthernetType()
                    .getType().getValue();
            ...
            EthernetMatchBuilder ethernetBuilder = new EthernetMatchBuilder()
                    .setEthernetType(new EthernetTypeBuilder().setType(
                            new EtherType(Long.valueOf(IPv4))).build());

            Ipv4MatchBuilder ipv4MatchBuilder = new Ipv4MatchBuilder();
            ipv4MatchBuilder.setIpv4Destination(new Ipv4Prefix(ipv4match
            ...
            actionBuilder.setAction(new GroupActionCaseBuilder()
                    .setGroupAction(actionBuilder_t.build()).build());
            ...
            WriteActions writeActions = new WriteActionsBuilder().setAction(
                    actions).build();
     ...
     *            next Objective.
     */
    private void processSimpleNextObjective(NextObjective nxtObject,
            InstanceIdentifier<Node> nodeID) {
        List<TrafficTreatment> tts = nxtObject.getTrafficTreatment();

        if (tts.size() != 1) {
            LOG.warn("Next Objective of type simple shoudl only have "
                    + "a single traffic treatment.");
            return;
        }
        ...
        DriverUtil.addGroup(this.salGroupService, tt.getAction(), nodeID,
                nxtObject.getNextId());
    }
    ...
    private org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.tables.table.Flow createGotoFilterRule(
        ...
        try {
            ...
            EthernetDestination dl_dst = filterObj.getEthernetMatch()
            ...
            Long ethType = filterObj.getEthernetMatch().getEthernetType()
                    .getType().getValue();
            ;
            ...
            ethMatchBuilder.setEthernetDestination(dl_dst).setEthernetType(
            ...
            vlanMatchBuilder.setVlanId(new VlanIdBuilder()
                    .setVlanId(vlanID.getVlanId()).setVlanIdPresent(true)
                    .build());
            ...
            ApplyActions applyActions = new ApplyActionsBuilder().setAction(
                    actions).build();
            GoToTableBuilder gotoInst = new GoToTableBuilder()
                    .setTableId(FIB_TABLE);
            ...
            Instruction instruction = new InstructionBuilder()
                    ...
                    .setInstruction(
                            new ApplyActionsCaseBuilder().setApplyActions(
                                    applyActions).build()).build();

            Instruction instructionGoto = new InstructionBuilder()
                    .setOrder(1)
                    .setInstruction(
                            new GoToTableCaseBuilder().setGoToTable(
                                    gotoInst.build()).build()).build();

            // set the flow instruction.
            flowBuilder.setInstructions(new InstructionsBuilder()
                    .setInstruction(
                            ImmutableList.of(instruction, instructionGoto))
                    .build());
            flowBuilder.setMatch(matchBuilder.build()).setTableId(FILTER_TABLE)
    ...
    private org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.tables.table.Flow createPuntFilterRule(
        ...
        try {
            ...
            VlanId vlanID = filterObj.getVlanMatch().getVlanId();
            NodeConnectorId inPort = filterObj.getInPort();
            Ipv4Prefix ipDst = ((Ipv4Match) filterObj.getLayer3Match())
                    .getIpv4Destination();
            Long ethType = filterObj.getEthernetMatch().getEthernetType()
                    .getType().getValue();
            ;
            ...
            Ipv4Match ipv4Match = new Ipv4MatchBuilder().setIpv4Destination(
                    ipDst).build();
     ...
     */
    private void processFilterObjective(FilterObjective filterObj,
            InstanceIdentifier<Node> nodeID) {
        ...
        try {
            ...
            if (flowPunt != null && flowNxtTable != null) {
                DriverUtil.install_flows(this.salFlowService,
                        Collections.singletonList(flowNxtTable), nodeID);
                DriverUtil.install_flows(this.salFlowService,
                        Collections.singletonList(flowPunt), nodeID);
            }
