...
public class Router implements BindingAwareProvider, AutoCloseable,
RoutingService {
    ...
    public void onSessionInitiated(ProviderContext session) {
        ...
        routesWaitingOnArp = Multimaps.synchronizedSetMultimap(HashMultimap
                .<IpAddress, RouteEntry> create());

        bgpUpdatesExecutor = Executors
                .newSingleThreadExecutor(new ThreadFactoryBuilder()
                .setNameFormat("atrium-bgp-updates-%d").build());
     ...
     */
    @Override
    public void setServices(RoutingConfigService routingConfigSvc,
            BgpService bgpSvc, HostService hostService) {
    ...
    public Collection<RouteEntry> getRoutes4() {
        Iterator<KeyValuePair<RouteEntry>> it = ribTable4
    ...
    public Collection<RouteEntry> getRoutes6() {
        Iterator<KeyValuePair<RouteEntry>> it = ribTable6
                .getKeyValuePairsForKeysStartingWith("").iterator();
    ...
    private void updateMac(IpAddress ipAddress, AtriumMacAddress macAddress) {
        ...
        synchronized (this) {
            ...
            Set<RouteEntry> routesToPush = routesWaitingOnArp
                    .removeAll(ipAddress);
            ...
            for (RouteEntry routeEntry : routesToPush) {
                ...
                if (foundRouteEntry != null
                        && foundRouteEntry.nextHop().equals(
                                routeEntry.nextHop())) {
                    ...
                    submitFibEntries.add(new AtriumFibUpdate(AtriumFibUpdate.Type.UPDATE,
                            new AtriumFibEntry(routeEntry.prefix(), ipAddress,
                                    macAddress)));
                } else {
                    LOG.debug(
     ...
     */
    private AtriumFibEntry processRouteAdd(RouteEntry routeEntry,
            Collection<IpPrefix> withdrawPrefixes) {
        ...
        if (isIpPrefixLocal(routeEntry.prefix())) {
            ...
            // these
            LOG.debug("Own route {} to {}", routeEntry.prefix(),
                    routeEntry.nextHop());
            return null;
        }
        ...
        LOG.info("Sending request to host service for MAC resolution : {}",
                routeEntry.nextHop());
        ...
        if (nextHopMacAddress == null) {
            Host host = hostService.getHost(new HostId(routeEntry.nextHop()
                    .toString()));
            if (host != null) {
                HostNode hostNode = host.getHostNode();
                if (hostNode != null) {
                    List<Addresses> addresses = hostNode.getAddresses();
        ...
        if (nextHopMacAddress == null) {
            LOG.info("nextHopMacAddress not found in ip2Mac : {}",
                    routeEntry.nextHop());
            routesWaitingOnArp.put(routeEntry.nextHop(), routeEntry);
            return null;
        }

        LOG.info("Creating FIB entry : " + routeEntry.prefix() + ","
                + routeEntry.nextHop() + "," + nextHopMacAddress);
        return new AtriumFibEntry(routeEntry.prefix(), routeEntry.nextHop(),
                nextHopMacAddress);
    }
        ...
        synchronized (this) {
            ...
            for (RouteUpdate update : routeUpdates) {
                ...
                case UPDATE:
                    ...
                    AtriumFibEntry fib = processRouteAdd(update.routeEntry(),
                            withdrawPrefixes);
                    if (fib != null) {
                        fibUpdates
                        .add(new AtriumFibUpdate(AtriumFibUpdate.Type.UPDATE, fib));
                    }
            ...
            withdrawPrefixes.forEach(p -> fibWithdraws.add(new AtriumFibUpdate(
                    AtriumFibUpdate.Type.DELETE, new AtriumFibEntry(p, null, null))));
     ...
     * <code>addInternalHostListener<code> method. When
     * the internalHost event occurs, that object's appropriate
     * method is invoked.
     *
    ...
    class InternalHostListener implements HostListener {
        ...
        public void hostEventUpdate(HostEvent event) {
            ...
            case HOST_ADDED: {
                ...
                List<Addresses> addresses = host.getHostNode().getAddresses();
                for (Addresses address : addresses) {
                    AtriumMacAddress mac = AtriumMacAddress.valueOf(address.getMac()
                            .toString());
                    Ipv4Address ipv4Address = address.getIp().getIpv4Address();
                    Ipv6Address ipv6Address = address.getIp().getIpv6Address();
                    if (ipv4Address != null) {
                        updateMac(IpAddress.valueOf(ipv4Address.getValue()),
                                mac);
                    }
                    if (ipv6Address != null) {
                        updateMac(IpAddress.valueOf(ipv6Address.getValue()),
                                mac);
                    }
                }
                break;
    ...
    private void doUpdatesThread() {
        ...
        try {
            while (!interrupted) {
                try {
                    Collection<RouteUpdate> routeUpdates = routeUpdatesQueue
                            .take();
