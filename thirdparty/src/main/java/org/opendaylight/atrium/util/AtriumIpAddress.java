 ...
 */
public class AtriumIpAddress implements Comparable<AtriumIpAddress> {
    // IP Versions
     ...
     */
    protected AtriumIpAddress(Version version, byte[] value) {
    ...
    public boolean isIp4() {
        return (version() == AtriumIp4Address.VERSION);
    }
    ...
    public boolean isIp6() {
        return (version() == AtriumIp6Address.VERSION);
    }

    /**
     * Gets the {@link AtriumIp4Address} view of the IP address.
     *
     * @return the {@link AtriumIp4Address} view of the IP address if it is IPv4,
     * otherwise null
     */
    public AtriumIp4Address getIp4Address() {
        if (!isIp4()) {
            return null;
        }

        // Return this object itself if it is already instance of Ip4Address
        if (this instanceof AtriumIp4Address) {
            return (AtriumIp4Address) this;
        }
        return AtriumIp4Address.valueOf(octets);
    }

    /**
     * Gets the {@link AtriumIp6Address} view of the IP address.
     *
     * @return the {@link AtriumIp6Address} view of the IP address if it is IPv6,
     * otherwise null
     */
    public AtriumIp6Address getIp6Address() {
        if (!isIp6()) {
            return null;
        }

        // Return this object itself if it is already instance of Ip6Address
        if (this instanceof AtriumIp6Address) {
            return (AtriumIp6Address) this;
        }
        return AtriumIp6Address.valueOf(octets);
    }
     ...
     */
    public static AtriumIpAddress valueOf(int value) {
        byte[] bytes =
            ByteBuffer.allocate(INET_BYTE_LENGTH).putInt(value).array();
        return new AtriumIpAddress(Version.INET, bytes);
    }
     ...
     * @throws IllegalArgumentException if the arguments are invalid
     */
    public static AtriumIpAddress valueOf(Version version, byte[] value) {
        return new AtriumIpAddress(version, value);
    }
    ...
    public static AtriumIpAddress valueOf(Version version, byte[] value,
        ...
        return AtriumIpAddress.valueOf(version, bc);
    }
     ...
     */
    public static AtriumIpAddress valueOf(InetAddress inetAddress) {
        byte[] bytes = inetAddress.getAddress();
        if (inetAddress instanceof Inet4Address) {
            return new AtriumIpAddress(Version.INET, bytes);
        }
        if (inetAddress instanceof Inet6Address) {
            return new AtriumIpAddress(Version.INET6, bytes);
        }
     ...
     * @throws IllegalArgumentException if the arguments are invalid
     */
    public static AtriumIpAddress makeMaskPrefix(Version version, int prefixLength) {
        byte[] mask = makeMaskPrefixArray(version, prefixLength);
        return new AtriumIpAddress(version, mask);
    }
     ...
     */
    public static AtriumIpAddress makeMaskedAddress(final AtriumIpAddress address,
                                              int prefixLength) {
        if (address instanceof AtriumIp4Address) {
            AtriumIp4Address ip4a = (AtriumIp4Address) address;
            return AtriumIp4Address.makeMaskedAddress(ip4a, prefixLength);
        } else if (address instanceof AtriumIp6Address) {
            AtriumIp6Address ip6a = (AtriumIp6Address) address;
            return AtriumIp6Address.makeMaskedAddress(ip6a, prefixLength);
        } else {
            byte[] net = makeMaskedAddressArray(address, prefixLength);
            return AtriumIpAddress.valueOf(address.version(), net);
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if ((obj == null) || (!(obj instanceof AtriumIpAddress))) {
            return false;
        }
        AtriumIpAddress other = (AtriumIpAddress) obj;
     ...
     */
    public AtriumIpPrefix toIpPrefix() {

        if (isIp4()) {
            return AtriumIpPrefix.valueOf(new AtriumIpAddress(Version.INET, octets),
                                    AtriumIp4Address.BIT_LENGTH);
        } else {
            return AtriumIpPrefix.valueOf(new AtriumIpAddress(Version.INET6, octets),
                                    AtriumIp6Address.BIT_LENGTH);
        }
    }
     ...
     * @throws IllegalArgumentException if the prefix length is invalid
     */
    static byte[] makeMaskedAddressArray(final AtriumIpAddress addr,
                                         int prefixLength) {
        byte[] mask = AtriumIpAddress.makeMaskPrefixArray(addr.version(),
                                                    prefixLength);
