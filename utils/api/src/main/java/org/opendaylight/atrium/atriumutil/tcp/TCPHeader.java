 ...
 * https://java.net/projects/slamd/sources/svn/content/trunk/slamd/tools/LDAPDecoder/src/com/sun/snoop/TCPHeader.java?rev=226
...
public class TCPHeader {
	 ...
	 *            The acknowledgement number for this TCP header.
	 * @param dataOffset
	 *            The data offset for this TCP header, measured in 32-bit words.
	 * @param tcpFlags
	 *            The set of flags associated with this TCP header.
	 * @param window
	 *            The window for this TCP header.
	 * @param checksum
	 *            The checksum for this TCP header.
	 ...
	 *            The urgent pointer for this TCP header.
	 * @param optionBytes
	 *            The raw data associated with any options in this TCP header.
	 */
	public TCPHeader(int sourcePort, int destinationPort, int sequenceNumber, int ackNumber, int dataOffset,
			int tcpFlags, int window, int checksum, int urgentPointer, byte[] optionBytes) {
	 ...
	 *
	 * @param headerBytes
	 *            The byte array containing the header information to decode.
	 * @param offset
	 *            The position in the byte array at which to begin decoding.
	 *
	 * @return The decoded TCP header.
	 *
	 * @throws TCPDecodeException
	 *             If a problem occurs while trying to decode the TCP header.
	 */
	public static TCPHeader decodeTCPHeader(byte[] headerBytes, int offset) throws TCPDecodeException {
		...
		if ((offset + 20) > headerBytes.length) {
			throw new TCPDecodeException(
					"Provided byte array is not large enough to " + "hold the minimum TCP header size of 20 bytes.");
		}

		int sourcePort = TCPDecoder.byteArrayToInt(headerBytes, offset, 2);
		int destinationPort = TCPDecoder.byteArrayToInt(headerBytes, offset + 2, 2);
		int sequenceNumber = TCPDecoder.byteArrayToInt(headerBytes, offset + 4, 4);
		int ackNumber = TCPDecoder.byteArrayToInt(headerBytes, offset + 8, 4);

		int dataOffset = ((headerBytes[offset + 12] >>> 4) & 0x0F);
		if ((offset + (dataOffset * 4)) > headerBytes.length) {
			throw new TCPDecodeException("Provided byte array is not large enough to "
					+ "hold the decoded TCP header size of " + (dataOffset * 4) + " bytes.");
		}

		int tcpFlags = (headerBytes[offset + 13] & 0x3F);
		int window = TCPDecoder.byteArrayToInt(headerBytes, offset + 14, 2);
		int checksum = TCPDecoder.byteArrayToInt(headerBytes, offset + 16, 2);
		int urgentPointer = TCPDecoder.byteArrayToInt(headerBytes, offset + 18, 2);

		byte[] optionBytes;
		if (dataOffset > 5) {
			optionBytes = new byte[(dataOffset - 5) * 4];
			System.arraycopy(headerBytes, offset + 20, optionBytes, 0, optionBytes.length);
		} else {
			optionBytes = new byte[0];
		}

		return new TCPHeader(sourcePort, destinationPort, sequenceNumber, ackNumber, dataOffset, tcpFlags, window,
				checksum, urgentPointer, optionBytes);
	}

	/**
	 * Retrieves the source port for this TCP packet. The port will be encoded
