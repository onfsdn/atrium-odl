...
public class AtriumTCPHeader {
	 ...
	 */
	public AtriumTCPHeader(int sourcePort, int destinationPort, int sequenceNumber, int ackNumber, int dataOffset,
	 ...
	 */
	public static AtriumTCPHeader decodeTCPHeader(byte[] headerBytes, int offset) throws AtriumTCPDecodeException {
		...
		if ((offset + 20) > headerBytes.length) {
			throw new AtriumTCPDecodeException(
					"Provided byte array is not large enough to " + "hold the minimum TCP header size of 20 bytes.");
		}

		int sourcePort = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset, 2);
		int destinationPort = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 2, 2);
		int sequenceNumber = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 4, 4);
		int ackNumber = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 8, 4);

		int dataOffset = ((headerBytes[offset + 12] >>> 4) & 0x0F);
		if ((offset + (dataOffset * 4)) > headerBytes.length) {
			throw new AtriumTCPDecodeException("Provided byte array is not large enough to "
					+ "hold the decoded TCP header size of " + (dataOffset * 4) + " bytes.");
		}

		int tcpFlags = (headerBytes[offset + 13] & 0x3F);
		int window = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 14, 2);
		int checksum = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 16, 2);
		int urgentPointer = AtriumTCPDecoder.byteArrayToInt(headerBytes, offset + 18, 2);
		...
		return new AtriumTCPHeader(sourcePort, destinationPort, sequenceNumber, ackNumber, dataOffset, tcpFlags, window,
