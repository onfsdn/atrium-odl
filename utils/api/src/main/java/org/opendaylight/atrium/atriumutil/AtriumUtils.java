...
public class AtriumUtils {
    ...
    public static String getDpnFromNodeConnectorId(NodeConnectorId portId) {
        ...
        String[] split = portId.getValue().split(
                AtriumConstants.OF_URI_SEPARATOR);
        ...
        try {
            nodesDataObjectOptional = readOnlyTransaction
                    .read(LogicalDatastoreType.OPERATIONAL,
                            nodesInsIdBuilder.build()).get();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
        }
        if (nodesDataObjectOptional != null
                && nodesDataObjectOptional.isPresent()) {
    ...
    public static BigInteger getDpnIdFromNodeName(String sMdsalNodeName) {
        String sDpId = sMdsalNodeName
                .substring(sMdsalNodeName.lastIndexOf(":") + 1);
     ...
     */
    public static NodeConnector getNodeConnector(DataBroker dataBroker,
        ...
        try {
            Optional<NodeConnector> dataObjectOptional = readOnlyTransaction
                    .read(LogicalDatastoreType.OPERATIONAL,
                            (InstanceIdentifier<NodeConnector>) ncRef
                            .getValue()).get();
            if (dataObjectOptional.isPresent())
                nc = dataObjectOptional.get();
        } catch (Exception e) {
            throw new RuntimeException(
                    "Error reading from configuration store, node connector : "
                            + ncRef, e);
        }
        return nc;
     ...
     */
    public static NodeConnectorRef getNodeConnRef(final NodeId nodeId,
            final Long port) {
     ...
     * @return the ether match
     */
    public static EthernetMatch getEtherMatch(MacAddress macAddress,
            boolean isSrc) {
        if (isSrc) {
            return new EthernetMatchBuilder().setEthernetSource(
                    new EthernetSourceBuilder().setAddress(macAddress).build())
                    .build();
        } else {
            return new EthernetMatchBuilder().setEthernetDestination(
                    new EthernetDestinationBuilder().setAddress(macAddress)
                    .build()).build();
        }
    }

    public static EthernetMatch getEtherMatch(MacAddress macAddress,EthernetType etherType,
            boolean isSrc) {
        if (isSrc) {
            return new EthernetMatchBuilder().setEthernetType(etherType).setEthernetSource(
                    new EthernetSourceBuilder().setAddress(macAddress).build())
                    .build();
        } else {
            return new EthernetMatchBuilder().setEthernetType(etherType).setEthernetDestination(
                    new EthernetDestinationBuilder().setAddress(macAddress)
                    .build()).build();
        }
    }
    ...
    public static Ipv4Match getL3Match(InterfaceIpAddress intfAddr,
            boolean isSrc) {

        if (isSrc) {
            return new Ipv4MatchBuilder().setIpv4Source(
                    new Ipv4Prefix(IpPrefix.valueOf(intfAddr.ipAddress(), 32)
                            .toString())).build();
        } else {
            return new Ipv4MatchBuilder().setIpv4Source(
                    new Ipv4Prefix(IpPrefix.valueOf(intfAddr.ipAddress(), 32)
    ...
    public static IpMatch getIpMatchType() {
    ...
    public static Layer3Match createLayer3Match(IpPrefix ipPrefix, boolean isSrc) {
        if (ipPrefix.isIp4()) {
            if (isSrc) {
                return new Ipv4MatchBuilder().setIpv4Source(
                        new Ipv4Prefix(ipPrefix.getIp4Prefix().toString()))
                        .build();
            } else {
                return new Ipv4MatchBuilder().setIpv4Destination(
                        new Ipv4Prefix(ipPrefix.getIp4Prefix().toString()))
                        .build();
            }
        } else {
            if (isSrc) {
                return new Ipv6MatchBuilder().setIpv6Source(
                        new Ipv6Prefix(ipPrefix.getIp6Prefix().toString()))
                        .build();
            } else {
                return new Ipv6MatchBuilder().setIpv6Destination(
                        new Ipv6Prefix(ipPrefix.getIp6Prefix().toString()))
                        .build();
            }
        }
