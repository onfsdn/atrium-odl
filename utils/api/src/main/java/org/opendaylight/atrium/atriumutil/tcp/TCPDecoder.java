 ...
 * https://java.net/projects/slamd/sources/svn/content/trunk/slamd/tools/LDAPDecoder/src/com/sun/snoop/SnoopDecoder.java?rev=226
...
public class TCPDecoder {
	 ...
	 *             header.
	 */
	public TCPDecoder(InputStream inputStream) throws IOException, TCPDecodeException {
		...
		if (!byteArraysAreEqual(idBytes, SNOOP_HEADER_BYTES)) {
			throw new TCPDecodeException("Input does not begin with the appropriate " + "snoop header");
		}
		...
		if (snoopVersion != 2) {
			throw new TCPDecodeException("Only snoop version 2 capture files are "
		...
		if ((dataLinkType < 0) || (dataLinkType >= 10)) {
			throw new TCPDecodeException("Invalid data link type (" + dataLinkType + ")");
		}
	}
