...
/**
 * Class for handling the BGP peer sessions. There is one instance per each BGP
 * peer session.
 */
public class BgpSession extends SimpleChannelHandler {
	 ...
	 *
	 * @param bgpSessionManager
	 *            the BGP Session Manager to use
	 */
	...
	public boolean isAs4OctetCapable() {
		return remoteInfo.as4OctetCapability() && localInfo.as4OctetCapability();
	}
	 ...
	 *
	 * @param prefix
	 *            the IPv4 prefix of the route to search for
	 ...
	 *
	 * @param prefix
	 *            the IPv6 prefix of the route to search for
	...
	/**
	 * Finds a BGP routing entry for a prefix in the BGP RIB-IN. The prefix can
	 * be either IPv4 or IPv6.
	 *
	 * @param prefix
	 *            the IP prefix of the route to search for
	 ...
	 * <p>
	 * NOTE: We use this method to avoid the Netty's asynchronous closing of a
	 * channel.
	 * </p>
	 * 
	...
	public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent channelEvent) {
		...
		log.debug("BGP Session Connected from {} on {}", remoteInfo.address(), localInfo.address());
		if (!bgpSessionManager.peerConnected(this)) {
			log.debug("Cannot setup BGP Session Connection from {}. Closing...", remoteInfo.address());
			ctx.getChannel().close();
		}
	...
	public void channelDisconnected(ChannelHandlerContext ctx, ChannelStateEvent channelEvent) {
		log.debug("BGP Session Disconnected from {} on {}", ctx.getChannel().getRemoteAddress(),
	...
	public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) {
		log.debug("BGP Session Exception Caught from {} on {}: {}", ctx.getChannel().getRemoteAddress(),
				ctx.getChannel().getLocalAddress(), e);
		processChannelDisconnected();
	}
	...
	private void processChannelDisconnected() {
		...
		BgpRouteSelector bgpRouteSelector = bgpSessionManager.getBgpRouteSelector();
	...
	void restartKeepaliveTimer(ChannelHandlerContext ctx) {
		...
		if (localInfo.holdtime() != 0) {
			localKeepaliveInterval = Math.max(localInfo.holdtime() / BgpConstants.BGP_KEEPALIVE_PER_HOLD_INTERVAL,
		...
		keepaliveTimeout = timer.newTimeout(new TransmitKeepaliveTask(ctx), localKeepaliveInterval, TimeUnit.SECONDS);
	}
	...
	void restartSessionTimeoutTimer(ChannelHandlerContext ctx) {
		...
		sessionTimeout = timer.newTimeout(new SessionTimeoutTask(ctx), remoteInfo.holdtime(), TimeUnit.SECONDS);
	}
